import { UserSecurityContext, UnauthenticatedContext } from "../domain/security-context";
...
export class TokenAuthenticatorImpl implements TokenAuthenticator {
    ...
    constructor(tokenParser: TokenConverter, tokenDao: TokenDao, secureHash: SecureHash) {
    ...
    public async authenticateToken(token: string): Promise<UserSecurityContext> {
        ...
        if (authToken != null && await this._secureHash.verifyHash(tokenValue, authToken.hashToken)) {
            return {
